	...
	void draw_GUI() {
		...
		if(ImGui::CollapsingHeader("Headset visualization options", ImGuiTreeNodeFlags_DefaultOpen))
		{
			...
			if(ImGui::Button("Zero orientation")){
				const pose_type fast_pose = pp->get_fast_pose(0.1/60.0);
		...
		if(ImGui::Button("Calculate new orientation offset")){
			const pose_type pose = pp->get_fast_pose(0.1/60.0);
	...
	void _p_one_iteration() override {
		{
			...
			if(pp->fast_pose_reliable()) {
				...
				// std::cout << "ASDFAJSKDFJ " << pose.orientation.w() << " " << pose.orientation.x() << " " << pose.orientation.y() << " " << pose.orientation.z() << std::endl;
				// std::cout << "TSETSETSET " << combinedQuat.w() << " " << combinedQuat.x() << " " << combinedQuat.y() << " " << combinedQuat.z() << std::endl;
				// std::cout << "SDFTHK " << offsetQuat.w() << " " << offsetQuat.x() << " " << offsetQuat.y() << " " << offsetQuat.z() << std::endl;
			...
			Eigen::Vector3f optionalOffset = (follow_headset && pp->fast_pose_reliable())
				? (pp->get_fast_pose(0.1/60.0).position + tracking_position_offset)
